...
) : IEnvironmentConfigurationDetector
{
    ...
    {
        var developerCommandPromptPath =
            await developerCommandPromptDetector.GetDeveloperCommandPromptAsync();
        var variables = await process.RunAndGetEnvironmentVariablesAsync(
            developerCommandPromptPath,
            token
        );
